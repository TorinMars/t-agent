...
- 顶部快速链接、PWA 安装；Web `/web` 和手机 H5 `/h5` 共用任务数据及身份验证器鉴权，手机首页自动进入 H5。
- H5 支持任务列表/详情切换、触屏“更多操作”、文档、待办和终端快捷键（Esc、Tab、Ctrl+C、方向键）。
...
- Client 不使用用户名密码，Web 与 H5 必须先绑定身份验证器，再以 6 位 TOTP 动态验证码登录；未绑定不能使用业务页面或浏览器接口。
- 首次绑定需要 Client 启动日志中的初始化码（15 分钟有效，重启重新生成），避免局域网其他人抢先绑定。绑定密钥加密保存，恢复码仅保存哈希；会话有效期 12 小时，支持主动退出、验证码防重放和认证限流。
...
新安装和旧版本升级初始都没有身份验证器绑定。再次进入 Web 或 H5 时会提示“必须绑定身份验证器”，未绑定不能访问任务、设置或终端；没有跳过绑定的免登录入口。

1. 从 Client 服务启动日志获取初始化码，15 分钟内在绑定页输入。Linux 可查看 `journalctl -u t-agent.service -n 50`；手动运行时查看启动终端。macOS 的日志位置可在 `~/Library/LaunchAgents/com.tagent.client.plist` 的 `StandardOutPath` 中查看。码过期时重启 Client 服务，已绑定后重启不会取消绑定。
...
4. 手机与 Client 在同一局域网时，将 Client `.env` 的 `HOST` 改为 `0.0.0.0` 并重启服务。手机浏览器打开 `http://电脑的局域网IP:3000/h5`（端口以实际配置为准）；电脑访问 `/web`。直接访问首页 `/` 会对手机自动跳转到 H5。

生产或公网使用必须通过 HTTPS/WSS 反向代理，并用防火墙限制来源；不要将 HTTP 原始端口直接暴露公网。身份验证器登录会话 12 小时有效，新设备、会话过期或主动退出后需要重新验证。登录设置中可更换身份验证器；丢失验证器时使用恢复码登录并重新绑定，更换后旧验证器、旧恢复码和其他设备会话立即失效。退出登录只断开网页终端连接，不终止服务端正在运行的程序。
...
任务详情顶部可切换技术方案、README、AGENT.md、待办和终端。终端会以该任务的工作路径作为当前目录。终端顶部提供三个操作：

- “重新打开”只重新建立浏览器连接，保留当前 Shell 进程、所在目录和历史。
- “关闭当前终端”会终止当前 Shell 及其中运行的程序，并保留已记录的输出历史。
...
连接成功后，顶部会出现新的 Engine 标签。每个标签会显示该 Engine 的当前版本；Client 启动时会自动探测远程版本，并在一分钟内对重复加载做节流。切换标签即可查看对应服务器的任务。
