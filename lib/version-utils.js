...
function githubVersionUrlFromRemote(remote, branch = 'main') {
  ...
  if (!match) return null;
  return `https://api.github.com/repos/${match[1]}/${match[2]}/contents/VERSION.json?ref=${encodeURIComponent(branch)}`;
}
...
function githubVersionUrlFromRepository(repository, ref = 'main') {
  ...
  return `https://api.github.com/repos/${repositoryValue}/contents/VERSION.json?ref=${encodeURIComponent(ref)}`;
}
